from src.processadoras.consigfacil.convenios.teresina.locators import TeresinaLocators
...
class ConvenioTeresina:
    ...
    def login(self):
        try:
            ...
            self.driver.find_element(*TeresinaLocators.CAMPO_SENHA).send_keys(self.second_password)
            ...
            try:
                WebDriverWait(self.driver, 1).until(
                    EC.presence_of_element_located(TeresinaLocators.BOTAO_CONFIRMA_LEITURA))
                return True
            except:
            ...
            while True:
                try:
                    WebDriverWait(self.driver, 1).until(
                        EC.presence_of_element_located(TeresinaLocators.CAMPO_LOGIN)).send_keys(self.user)
                    self.driver.find_element(*TeresinaLocators.CAMPO_SENHA).send_keys(self.second_password)
                    CF_CAPTCHA_RESOLVER = input ("Digite o Captcha: ")
                    self.driver.find_element(*TeresinaLocators.CAMPO_CAPTCHA).send_keys(CF_CAPTCHA_RESOLVER)
                    self.driver.find_element(*TeresinaLocators.CAMPO_CAPTCHA).send_keys(Keys.ENTER)
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(TeresinaLocators.ABA_RELATORIO))
                    return True
                except:
                    print("Captcha digitado incorretamente, tentar novamente")
                    time.sleep(0.5)
    ...
    def troca_senha(self):
        try:
            try:
                self.driver.find_element(*TeresinaLocators.CAMPO_SENHA_TROCA).click()
                self.driver.find_element(*TeresinaLocators.CAMPO_SENHA_TROCA).send_keys(self.second_password)
                time.sleep(0.5)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(TeresinaLocators.CAMPO_NOVA_SENHA_TROCA)).send_keys(self.password)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(TeresinaLocators.CAMPO_NOVA_SENHA_CONFIRMA)).send_keys(self.password)
                time.sleep(0.3)
                WebDriverWait(self.driver, 1.0).until(
                    EC.element_to_be_clickable(TeresinaLocators.BODY)).click()
                self.driver.find_element(*TeresinaLocators.BODY).send_keys(Keys.PAGE_DOWN)
                time.sleep(0.1)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(TeresinaLocators.BOTAO_ENTRAR_TROCA_SENHA)).click()
                return True
            except Exception as e:
                print(f"Erro: {e}")
                return False
    ...
    def opcoes_relatorio(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(TeresinaLocators.DATA_INICIO)).send_keys(data.DATA_OPERACOES)
            self.driver.find_element(*TeresinaLocators.DATA_FIM).send_keys(data.DATA_FINAL)
            
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(TeresinaLocators.CAMPO_SERVICOS)).click()
            WebDriverWait(self.driver, 2.5).until(
                EC.element_to_be_clickable(TeresinaLocators.OPCAO_SAQUE)).click()
