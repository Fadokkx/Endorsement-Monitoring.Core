...
class ConvenioIpatinga:
    ...
    def login(self):
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(IpatingaLocators.CAMPO_LOGIN)
            )
            self.driver.find_element(*IpatingaLocators.CAMPO_LOGIN).send_keys(self.user)
            ...
            self.driver.find_element(*IpatingaLocators.BOTAO_LOGIN).click()
            time.sleep(1)
            return True
    ...
    def navega_menu(self):
        try:
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(IpatingaLocators.ABA_RELATORIO)
            )
            self.driver.find_element(*IpatingaLocators.ABA_RELATORIO).click()
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(IpatingaLocators.SELEC_RELATORIO)
            )
            self.driver.find_element(*IpatingaLocators.SELEC_RELATORIO).click()
            return True
        except Exception as e:
            print(f"Erro: {e}")
    ...
    def opcoes_relatorio(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(IpatingaLocators.DATA_INICIO)
            )
            self.driver.find_element(*IpatingaLocators.DATA_INICIO).send_keys(data.DATA_OPERACOES)
            self.driver.find_element(*IpatingaLocators.DATA_FIM).send_keys(data.DATA_FINAL)
            time.sleep(1)
            self.driver.find_element(By.XPATH, "/html/body").send_keys(Keys.PAGE_DOWN)
            time.sleep(1)
            self.driver.find_element(*IpatingaLocators.OPCAO_REL).click()
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(IpatingaLocators.TIPO_CSV)
            )
            self.driver.find_element(*IpatingaLocators.TIPO_CSV).click()
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(IpatingaLocators.BOTAO_GERAR)
            )
            return True
    ...
    def baixar_relatorio(self):
        try:
            ...
            self.driver.find_element(*IpatingaLocators.BOTAO_GERAR).click()
            time.sleep(2)
            return True
        except Exception as e:
            print (f"Erro: {e}")
