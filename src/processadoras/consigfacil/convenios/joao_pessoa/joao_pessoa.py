from src.processadoras.consigfacil.convenios.joao_pessoa.locators import JoaoPessoaLocators
...
class ConvenioJoaoPessoa:
        ...
        try:
            ...
            try:
                WebDriverWait(self.driver, 1).until(
                    EC.presence_of_element_located(JoaoPessoaLocators.ABA_RELATORIO))
                return True
            except:
    ...
    def troca_senha(self):
        try:
            try:
                ...
                self.driver.find_element(*JoaoPessoaLocators.CAMPO_SENHA_TROCA).send_keys(self.second_password)
                time.sleep(0.5)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(JoaoPessoaLocators.CAMPO_NOVA_SENHA_TROCA)).send_keys(self.password)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(JoaoPessoaLocators.CAMPO_NOVA_SENHA_CONFIRMA)).send_keys(self.password)
                time.sleep(0.3)
    ...
    def navega_menu(self):
        try:
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(JoaoPessoaLocators.ABA_RELATORIO)).click()
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(JoaoPessoaLocators.SELEC_RELATORIO)).click()
            return True
        except Exception as e:
            print(f"Erro: {e}")
    ...
    def opcoes_relatorio(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(JoaoPessoaLocators.DATA_INICIO)).send_keys(data.DATA_OPERACOES)
            ...
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(JoaoPessoaLocators.TIPO_CSV)).click()
            return True
    ...
    def baixar_relatorio(self):
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(JoaoPessoaLocators.BOTAO_GERAR)).click()
            time.sleep(1.5)
            try:
