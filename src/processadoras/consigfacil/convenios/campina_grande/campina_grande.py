from src.processadoras.consigfacil.convenios.campina_grande.locators import CampinaGrandeLocators
...
class ConvenioCampinaGrande:
        ...
        try:
            ...
            try:
                WebDriverWait(self.driver, 5).until(
                    EC.element_to_be_clickable(CampinaGrandeLocators.BOTAO_CONFIRMA_LEITURA)).click()
    ...
    def navega_menu(self):
        try:
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.ABA_RELATORIO)).click()
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.SELEC_RELATORIO)).click()
            return True
        except Exception as e:
            print(f"Erro: {e}")
    ...
    def opcoes_relatorio(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.presence_of_element_located(CampinaGrandeLocators.DATA_INICIO)).send_keys(data.DATA_OPERACOES)
            self.driver.find_element(*CampinaGrandeLocators.DATA_FIM).send_keys(data.DATA_FINAL)
            
            WebDriverWait(self.driver, 3).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.CAMPO_SERVICOS)).click()
            WebDriverWait(self.driver, 1.5).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.OPCAO_SAQUE)).click()
            
            time.sleep(0.1)
            self.driver.find_element(By.XPATH, "/html/body").send_keys(Keys.PAGE_DOWN)
            time.sleep(0.5)
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.OPCAO_REL)).click()
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.TIPO_CSV)).click()
            return True
    ...
    def baixar_relatorio(self):
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(CampinaGrandeLocators.BOTAO_GERAR)).click()
            time.sleep(1.5)
