...
class ConvenioCuiaba:
    ...
    def troca_senha(self):
        try:
            try:
                self.driver.find_element(*CuiabaLocators.CAMPO_SENHA_TROCA).click()
                self.driver.find_element(*CuiabaLocators.CAMPO_SENHA_TROCA).send_keys(self.second_password)
                time.sleep(0.5)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(CuiabaLocators.CAMPO_NOVA_SENHA_TROCA)).send_keys(self.password)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(CuiabaLocators.CAMPO_NOVA_SENHA_CONFIRMA)).send_keys(self.password)
                time.sleep(0.3)
                WebDriverWait(self.driver, 1.0).until(
                    EC.element_to_be_clickable(CuiabaLocators.BODY)).click()
                self.driver.find_element(*CuiabaLocators.BODY).send_keys(Keys.PAGE_DOWN)
                time.sleep(0.1)
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(CuiabaLocators.BOTAO_ENTRAR_TROCA_SENHA)).click()
                return True
            except Exception as e:
                print(f"Erro: {e}")
                return False
