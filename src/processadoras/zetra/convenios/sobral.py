...
class ConvenioSobral:
        ...
        try:
            ...
            try:
                WebDriverWait(self.driver, 1.5).until(
                            EC.presence_of_element_located(SobralLocators.MENU_PRINCIPAL))
                return True
            ...
            while True:
                try:
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(SobralLocators.CAMPO_USUARIO)).send_keys(self.user)
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(SobralLocators.BOTAO_CONTINUAR)).click()
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(SobralLocators.CAMPO_SENHA)).send_keys(self.second_password)
                    ZetraCaptchaResolver = input("Resolva o captcha e pressione Enter: ")
                    WebDriverWait(self.driver, 1).until(
                        EC.presence_of_element_located(SobralLocators.CAMPO_CAPTCHA)).send_keys(ZetraCaptchaResolver)
                    self.driver.find_element(*SobralLocators.CAMPO_CAPTCHA).send_keys(Keys.ENTER)
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(SobralLocators.MENU_PRINCIPAL))
                    return True
    ...
    def confirmacao_leitura(self):
        ...
        time.sleep(0.5)
        try:
            self.driver.find_element(*SobralLocators.RADIO_CONFIRMA_LEITURA).click()
            pg.hotkey('pagedown')
            time.sleep(0.5)
            ...
            try:
                try:
                    WebDriverWait(self.driver, 0.5).until(
                        EC.element_to_be_clickable(SobralLocators.RADIO_CONFIRMA_LEITURA2)).click()
                except:
                    pass
                ...
                try:
                    try:
                        WebDriverWait(self.driver, 0.5).until(
                            EC.element_to_be_clickable(SobralLocators.RADIO_CONFIRMA_LEITURA3)).click()
                        time.sleep(0.1)
                        pg.hotkey('pagedown')
                    except:
                        pass
                    ...
                    try:
                        try:
                            WebDriverWait(self.driver, 0.5).until(
                                EC.element_to_be_clickable(SobralLocators.RADIO_CONFIRMA_LEITURA4)).click()
                            time.sleep(0.1)
                        except:
                            pass
                        ...
                        try:
                            try:
                                WebDriverWait(self.driver, 0.5).until(
                                    EC.element_to_be_clickable(SobralLocators.RADIO_CONFIRMA_LEITURA5)).click()
                                time.sleep(0.1)
                            except:
                                pass
                            ...
                            try:
                                try:
                                    WebDriverWait(self.driver, 0.5).until(
                                        EC.element_to_be_clickable(SobralLocators.RADIO_CONFIRMA_LEITURA6)).click()
                                    time.sleep(0.1)
                                    pg.hotkey('pagedown')
                                except:
                                    pass
                                ...
                                try:
                                    WebDriverWait(self.driver, 1.5).until(
                                        EC.element_to_be_clickable(SobralLocators.BOTAO_CONFIRMA_LEITURA)).click()
    ...
    def opcoes_relatorios(self):
        try:
            WebDriverWait(self.driver, 15).until(
                EC.element_to_be_clickable(SobralLocators.DATA_INICIO)).send_keys(data.DATA_OPERACOES)
            self.driver.execute_script("document.body.style.zoom='60%'")
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(SobralLocators.DATA_FIM)).send_keys(data.DATA_FINAL)
            time.sleep(0.1)
            
            WebDriverWait(self.driver, 4).until(
                EC.element_to_be_clickable(SobralLocators.CAMPO_SERVICOS)).click()
            self.driver.find_element(*SobralLocators.CAMPO_SERVICOS).send_keys(Keys.ARROW_DOWN)
