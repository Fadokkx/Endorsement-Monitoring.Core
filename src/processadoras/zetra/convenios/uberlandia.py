...
class ConvenioUberlandia:
        ...
        try:
            ...
            try:
                WebDriverWait(self.driver, 1.5).until(
                            EC.presence_of_element_located(UberlandiaLocators.MENU_PRINCIPAL))
                return True
            ...
            while True:
                try:
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(UberlandiaLocators.CAMPO_USUARIO)).send_keys(self.user)
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(UberlandiaLocators.BOTAO_CONTINUAR)).click()
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(UberlandiaLocators.CAMPO_SENHA)).send_keys(self.second_password)
                    ZetraCaptchaResolver = input("Resolva o captcha e pressione Enter: ")
                    WebDriverWait(self.driver, 1).until(
                        EC.presence_of_element_located(UberlandiaLocators.CAMPO_CAPTCHA)).send_keys(ZetraCaptchaResolver)
                    self.driver.find_element(*UberlandiaLocators.CAMPO_CAPTCHA).send_keys(Keys.ENTER)
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(UberlandiaLocators.MENU_PRINCIPAL))
                    return True
    ...
    def troca_senha(self):
        ...
        try:
            ...
            self.driver.find_element(*UberlandiaLocators.BOTAO_VOLTA_TROCA_SENHA).click()
        except:
    ...
    def opcoes_relatorios(self):
        try:
            WebDriverWait(self.driver, 15).until(
                EC.element_to_be_clickable(UberlandiaLocators.DATA_INICIO)).send_keys(data.DATA_OPERACOES)
            self.driver.execute_script("document.body.style.zoom='60%'")
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(UberlandiaLocators.DATA_FIM)).send_keys(data.DATA_FINAL)
            time.sleep(0.1)
            WebDriverWait(self.driver, 4).until(
                EC.element_to_be_clickable(UberlandiaLocators.CAMPO_SERVICOS)).click()
            time.sleep(0.1)
            self.driver.find_element(By.XPATH, "/html/body").send_keys(Keys.PAGE_DOWN)
            time.sleep(0.1)
            self.driver.find_element(By.XPATH, "/html/body").send_keys(Keys.PAGE_DOWN)
            ...
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(UberlandiaLocators.OPCAO_CSV)).click()
            time.sleep(0.3)
