...
class ConvenioSbc:
    def __init__(self, driver: WebDriver):
        self.driver = driver
        self.url = os.getenv("ZETRA_SBC_URL")
        ...
        try:
            ...
            try:
                WebDriverWait(self.driver, 1.5).until(
                            EC.presence_of_element_located(SBCLocators.MENU_PRINCIPAL))
                return True
            ...
            while True:
                try:
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(SBCLocators.CAMPO_USUARIO)).send_keys(self.user)
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(SBCLocators.BOTAO_CONTINUAR)).click()
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(SBCLocators.CAMPO_SENHA)).send_keys(self.second_password)
                    ZetraCaptchaResolver = input("Resolva o captcha e pressione Enter: ")
                    WebDriverWait(self.driver, 1).until(
                        EC.presence_of_element_located(SBCLocators.CAMPO_CAPTCHA)).send_keys(ZetraCaptchaResolver)
                    self.driver.find_element(*SBCLocators.CAMPO_CAPTCHA).send_keys(Keys.ENTER)
                    WebDriverWait(self.driver, 1.5).until(
                        EC.presence_of_element_located(SBCLocators.MENU_PRINCIPAL))
                    return True
    ...
    def confirmacao_leitura(self):
        ...
        time.sleep(0.5)
        try:
            self.driver.find_element(*SBCLocators.RADIO_CONFIRMA_LEITURA).click()
            pg.hotkey('pagedown')
            time.sleep(0.5)
            ...
            try:
                try:
                    WebDriverWait(self.driver, 0.5).until(
                        EC.element_to_be_clickable(SBCLocators.RADIO_CONFIRMA_LEITURA2)).click()
                except:
                    pass
                ...
                try:
                    try:
                        WebDriverWait(self.driver, 0.5).until(
                            EC.element_to_be_clickable(SBCLocators.RADIO_CONFIRMA_LEITURA3)).click()
                        time.sleep(0.1)
                        pg.hotkey('pagedown')
                    except:
                        pass
                    ...
                    try:
                        try:
                            WebDriverWait(self.driver, 0.5).until(
                                EC.element_to_be_clickable(SBCLocators.RADIO_CONFIRMA_LEITURA4)).click()
                            time.sleep(0.1)
                        except:
                            pass
                        ...
                        try:
                            try:
                                WebDriverWait(self.driver, 0.5).until(
                                    EC.element_to_be_clickable(SBCLocators.RADIO_CONFIRMA_LEITURA5)).click()
                                time.sleep(0.1)
                            except:
                                pass
                            ...
                            try:
                                try:
                                    WebDriverWait(self.driver, 0.5).until(
                                        EC.element_to_be_clickable(SBCLocators.RADIO_CONFIRMA_LEITURA6)).click()
                                    time.sleep(0.1)
                                    pg.hotkey('pagedown')
                                except:
                                    pass
                                ...
                                try:
                                    WebDriverWait(self.driver, 1.5).until(
                                        EC.element_to_be_clickable(SBCLocators.BOTAO_CONFIRMA_LEITURA)).click()
