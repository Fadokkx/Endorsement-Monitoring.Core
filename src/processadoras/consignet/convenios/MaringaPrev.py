...
class ConvenioMaringaPrev:
            ...
            try:
                try:
                    WebDriverWait(self.driver, 2).until(
                            EC.element_to_be_clickable(MaringaPrevLocators.BOTAO_PERFIL)).click()
                    WebDriverWait(self.driver, 10).until(
                            EC.element_to_be_clickable(MaringaPrevLocators.BOTAO_TROCA_PERFIL)).click()
                    time.sleep(1)
                except:
                    self.driver.get(self.url)
                    WebDriverWait(self.driver, 10).until(
                        EC.element_to_be_clickable(MaringaPrevLocators.CAMPO_USER)).send_keys(self.user)
                    self.driver.find_element(*MaringaPrevLocators.BOTAO_CONTINUAR).click()
                    WebDriverWait(self.driver, 10).until(
                        EC.element_to_be_clickable(MaringaPrevLocators.CAMPO_SENHA)).send_keys(self.password)
                    self.driver.find_element(*MaringaPrevLocators.BOTAO_ENTRAR).click()
                    time.sleep(0.5)
                return True
    ...
    def selec_convenios(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(MaringaPrevLocators.CAMPO_BUSCA_CONVENIO)).send_keys("MARINGAPREV / MEUCASHCARD SAQUE")
            self.driver.find_element(*MaringaPrevLocators.OPCAO_CONVENIO).click()
            return True
        except Exception as e: 
            print(f"Erro {e}")
            return False
    ...
    def navega_menu(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(MaringaPrevLocators.ABA_RELATORIOS)).click()
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(MaringaPrevLocators.OPCAO_CONSIGNACOES)).click()
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(MaringaPrevLocators.OPCAO_CONSIGNACAO_MENSAL)).click()
            return True
    ...
    def baixa_relatorio(self):
        try:
            time.sleep(1)
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(MaringaPrevLocators.OPCAO_XLS)).click()
            
            try:  
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(MaringaPrevLocators.BOTAO_CONFIRMA_DOWNLOAD)).click()
            except:
                print("Sem necessidade de confirmar download")
  
            try:
                WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable(MaringaPrevLocators.BOTAO_FECHA_SUCESSO)).click()
            except:
