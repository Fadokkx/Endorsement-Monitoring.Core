...
class ConvenioAmazonas:
    def __init__(self, driver: WebDriver):
        self.driver = driver
        self.url = os.getenv("CONSIGLOG_AMAZONAS_URL")
        self.user = os.getenv("CONSIGLOG_AMAZONAS_USER")
        self.password = os.getenv("CONSIGLOG_AMAZONAS_PASS")
    ...
    def login(self):
        try:
            ...
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.CAMPO_USER)).send_keys(self.user)
            self.driver.find_element(*AmazonasLocators.BOTAO_CONTINUAR).click()
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.CAMPO_SENHA)).send_keys(self.password)
            self.driver.find_element(*AmazonasLocators.BOTAO_ENTRAR).click()
                        
            try:
                WebDriverWait(self.driver, 1.5).until(
                    EC.element_to_be_clickable(AmazonasLocators.BOTAO_LOGOUT_SESSAO_INATIV)).click()
            except:
    ...
    def selec_convenio(self):
        try:
            time.sleep(0.5)
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.CONVENIO_PREFDUQUE)).click()
            time.sleep(0.1)
            return True
        except Exception as e:
            print(f"Erro {e}")
    ...
    def navega_menu(self):
        try:
            try:
                while True:
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(AmazonasLocators.CONFIRMACAO_LEITURA)).click()
                    WebDriverWait(self.driver, 2).until(
                        EC.element_to_be_clickable(AmazonasLocators.FECHA_POPUP_CONF_LEITURA)).click()
                    time.sleep(0.5)
            except:
            ...
            try:
                WebDriverWait(self.driver, 10).until(
                    EC.element_to_be_clickable(AmazonasLocators.COOKIES_NOTIF)).click()        
            except:
    ...
    def opcoes_relatorio(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.BOTAO_TIPO_REL)).click()
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(AmazonasLocators.TIPO_ANDEFERIDO)).click()
            time.sleep(0.5)
            WebDriverWait(self.driver, 3).until(
                EC.element_to_be_clickable(AmazonasLocators.BOTAO_ORGAOS_SELEC)).click()
            WebDriverWait(self.driver, 3).until(
                EC.element_to_be_clickable(AmazonasLocators.CHECK_ALL_ORG)).click()
            time.sleep(0.5)
            try:
                self.driver.find_element(*AmazonasLocators.BOTAO_ORGAOS_SELEC).click()
            except Exception as e:
                print(f"Erro: {e}")
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.CAMPO_PER_INI)).send_keys(data.DATA_INICIO)
            time.sleep(0.1)
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(AmazonasLocators.CAMPO_PER_FIM)).send_keys(data.DATA_FINAL)
            time.sleep(0.5)
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.BOTAO_SERVICO)).click()
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(AmazonasLocators.CHECK_ALL_SERVICES)).click()
            time.sleep(0.5)
            try:
                self.driver.find_element(*AmazonasLocators.BOTAO_SERVICO).click()
            except Exception as e:
                print(f"Erro: {e}")                
            return True
    ...
    def download_relatorio(self):
        try:
            WebDriverWait(self.driver, 10).until(
                EC.element_to_be_clickable(AmazonasLocators.BOTAO_OPCOES_REL)).click()
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(AmazonasLocators.OPCAO_XLSX)).click()
            
            try:
                self.driver.find_element(*AmazonasLocators.BOTAO_OPCOES_REL).click()
            except Exception as e:
                print(f"Erro: {e}")
                
            WebDriverWait(self.driver, 5).until(
                EC.element_to_be_clickable(AmazonasLocators.BOTAO_GERAR_REL)).click()
            time.sleep(3.5)
            return True
